...
exports.monthlyMember = async(req,res)=>{
    ...
    const member=await Member.find({gym:req.gym._id,
        nextBillDate: {
            $gte : next4Days,   //greater than or equal to 4 days
            $lte : next7Days     //less than or equal to 7 days
        }
    });
    ...
    exports.inActiveMember = async(req,res)=>{
        try{
            ...
            res.status(200).json({
                message:member.length?"Fetched Inactive Members SuccessFully":"No Such Member has been Pending",
                members:member,
                totalMembers:member.length
            })  
        }catch(err){
