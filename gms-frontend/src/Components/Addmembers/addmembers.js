...
const Addmembers = () => {
  ...
  try{
  const response=await axios.post("https://api.cloudinary.com/v1_1/dq0dqfo54/image/upload",data);
...
const fetchMembership=async()=>{
  await axios.get('https://gym-management-9zrp.onrender.com/plans/get-membership', {withCredentials:true}).then((response)=>{
...
await axios.post('https://gym-management-9zrp.onrender.com/members/register-member',inputField,{withCredentials:true}).then((res)=>{
...
<input value={inputField.name} onChange={(event) => { handleOnChange(event, "name") }} placeholder='Name of Joinee' type='text' className=' border-2 w-[90%] pl-3 pr-3 pt-2 pb-2 border-slate-400 rounded-md h-12'/>
